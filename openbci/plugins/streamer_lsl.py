...
class StreamerLSL(plugintypes.IPluginExtended):
    ...
    def activate(self):
        ...
        print(
            f"Creating LSL stream for EEG. Name:{eeg_stream}- ID:{eeg_id}- data type: float32.{str(self.eeg_channels)}channels at{str(self.sample_rate)}Hz."
        )
        ...
        print(
            f"Creating LSL stream for AUX. Name:{aux_stream}- ID:{aux_id}- data type: float32.{str(self.aux_channels)}channels at{str(self.sample_rate)}Hz."
        )
        ...
        if self.imp_channels > 0:
            print(
                f"Creating LSL stream for Impedance. Name:{imp_stream}- ID:{imp_id}- data type: float32.{str(self.imp_channels)}channels at{str(self.sample_rate)}Hz."
            )
